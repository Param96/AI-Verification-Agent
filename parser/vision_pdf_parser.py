    ...
    try:
        ...
        for page_num in range(len(doc)):
                ...
                if "lines" in b:
                    for line in b["lines"]:
                        for span in line["spans"]:
                            q = get_quadrant(
                                span["bbox"][0], span["bbox"][1], width, height
                            )
                            ...
                            quadrants[q]["spans"].append({"text": text, "y": y, "x": x})
            ...
            for q_id, data in quadrants.items():
                ...
                full_text = " ".join(
                    [s["text"].strip() for s in data["spans"] if s["text"].strip()]
                )
                ...
                duration_match = re.search(
                    r"Duration:\s*(\S+)", full_text, re.IGNORECASE
                )
                language_match = re.search(
                    r"Language:\s*(\S+)", full_text, re.IGNORECASE
                )
                skills_match = re.search(
                    r"Skills:\s*(.*?)(?:Link to course|$)", full_text, re.IGNORECASE
                )
                domain_match = re.search(
                    r"Domain:\s*(.*?)(?:Cost:|Mode:|Country:|Duration:|Language:|Skills:|$)",
                    full_text,
                    re.IGNORECASE,
                )
                header_text = (
                    full_text.split("Cost:")[0].strip()
                    if "Cost:" in full_text
                    else full_text[:100]
                )
                ...
                if domain_match:
                    ...
                    "FREE COURSE" in full_text.upper() or extracted_domain == "Unknown"
                ):
                    extracted_domain = (
                        page_domain if page_domain != "Unknown" else "General"
                    )

                # Filter out legend/footer boxes that are not actual courses
                if (
                    "NIRF Ranked Institute" in header_text
                    or "Free To Audit Course" in header_text
                ):
                    continue
                if (
                    not (cost_match or mode_match or duration_match or country_match)
                    and not data["link"]
                ):
                    continue
