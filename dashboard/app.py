...
col2.metric(
    "Valid Entries",
    f"{valid_rows}",
    f"{(valid_rows/total_rows*100):.1f}%" if total_rows > 0 else "0%",
)
...
for idx, row in filtered_df.iterrows():
    ...
    with st.expander(
        f"[{row['verification_status']}] {row['institute_name']} - {row['course_name']}"
    ):
